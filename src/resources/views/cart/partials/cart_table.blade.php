    ...
    @foreach($cart_products as $cart_item)
        <div class="col-sm-12 col-md-12" id="Cart-Products-Container">
            <div class="col-sm-3 col-md-3 center-on-small-only">
                <a href="{{ route('show.product', $cart_item->products->product_name) }}">
                    ...
                    @if ($cart_item->products->photos->count() === 0)
                        <img src="/cleaningservice/src/public/images/no-image-found.jpg" alt="No Image Found Tag" id="Product-similar-Image" style="width: 100px; height: 100px;" >
                    @else
                        @if ($cart_item->featuredPhoto)
                            <img src="/cleaningservice/{{ $cart_item->featuredPhoto->thumbnail_path }}" alt="Photo ID: {{ $cart_item->featuredPhoto->id }}" style="width: 100px; height: 100px;" />
                        @elseif(!$cart_item->featuredPhoto)
                            <img src="/cleaningservice/{{ $cart_item->products->photos->first()->thumbnail_path}}" alt="Photo" style="width: 100px; height: 100px;" />
                        @else
                            N/A
                        @endif
                    @endif
            ...
            <div class="col-sm-2 col-md-2" id="Carts-Sub-Containers">
                {{-- dd($cart_item->products->user()->find($user->id)->pivot->price) --}}
                @if($cart_item->products->user()->find($user->id))
                    <div class="discount light-300 black-text medium-500" id="Product_Reduced-Price"><s>￥ {{ $cart_item->products->price }}</s></div>
                    <div class="light-300 red-text medium-500" id="Product_Reduced-Price">￥ {{  $cart_item->products->user()->find($user->id)->pivot->price}}</div>
                @else
            ...
            <div class="col-sm-3 col-md-3" id="Carts-Sub-Containers">
                <form action="/cleaningservice/cart/update" method="post" class="form-inline">
                    {!! csrf_field() !!}
                    <input type="hidden" name="product" value="{{ $cart_item->products->id}}" />
                    <input type="hidden" name="cart_id" value="{{ $cart_item->id}}" />
                    <div class="form-group">
                        <input type="number" id="Product_QTY" class="form-control" name="qty" value="10">
