            ...
            <div class="col-md-12">
                ...
                <div class="col-xs-12 col-sm-12 col-md-8 gallery">
                    @if ($product->photos->count() === 0)
                        ...
                        <img src="/cleaningservice/src/public/images/no-image-found.jpg" alt="No Image Found Tag" id="Product-similar-Image">
                    @else
                        @foreach ($product->photos->slice(0, 8) as $photo)
                            <div class="col-xs-6 col-sm-4 col-md-3 gallery_image text-center">
                                <a href="/cleaningservice/{{ $photo->path }}" data-lity>
                                    <img src="/cleaningservice/{{ $photo->thumbnail_path }}" alt="Photo ID: {{ $photo->id  }}" data-id="{{ $photo->id }}" class="img-thumbnail">
                                </a>
                            </div>
                        @endforeach
                    @endif
                            ...
                            <a href="{{ route('show.product', $similar->product_name) }}">
                                ...
                                @else
                                    @if ($similar->featuredPhoto)
                                        <p id="Similar-Title">{{ str_limit($similar->product_name, $limit = 28, $end = '...') }}</p>
                                        <img src="/cleaningservice/{{ $similar->featuredPhoto->thumbnail_path }}" alt="Photo ID: {{ $similar->featuredPhoto->id }}" id="Product-similar-Image" />
                                    @elseif(!$similar->featuredPhoto)
                                        <p id="Similar-Title">{{ $similar->product_name }}</p>
                                        <img src="/cleaningservice/{{ $similar->photos->first()->thumbnail_path}}" alt="Photo" id="Product-similar-Image" />
                                    @else
                                        N/A
                                    @endif
                                @endif
