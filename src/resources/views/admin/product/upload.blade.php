...
@section('content')
    ...
    <div class="container">
        ...
        <div class="col-md-12">
            ...
            @else
                ...
                @else
                        ...
                        {!! Form::model($product, ['route' => ['store.admin.products', $products->id], 'method' =>'POST', 'files' => true, 'class'=>'dropzone', 'id'=>'addProductImages', 'enctype'=>'multipart/form-data'])!!}
                        {{ csrf_field() }}
                        {!! Form::close() !!}
                            ...
                            <div class="col-xs-6 col-sm-3 col-md-3 gallery_image">
                                ...
                                @if (Auth::user()->admin == 1)
                                    <a href="/store/{{ $photo->path }}" data-lity>
                                        <img src="{{ url($photo->thumbnail_path) }}" alt="" data-id="{{ $photo->id }}">
                                    </a>
                                @else
                                    <div class="img-wrap">
                                        <form method="post" action="/cleaningservice/admin/products/photos/{{ $photo->id }}">
                                            {!! csrf_field() !!}
        ...
        <div class="col-md-12 gallery" id="gallery">
            ...
            <form method="post" action="/cleaningservice/admin/products/add/featured/{{ $products->id }}">
                {!! csrf_field() !!}
